  ...
  async runAgent(input: GatewayInput) {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        input: [{ role: 'system', content: this.instructions(input.agent,input.context) }, { role: 'user', content: JSON.stringify({ snapshot: input.inputSnapshot, messages: input.messages ?? [] }) }],
