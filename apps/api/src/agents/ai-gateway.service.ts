...
import { GENERAL_CHAT_FUNCTION_NAMES, generalChatToolDefinitions } from './general-chat-tool-contracts';
...
export class AiGatewayService {
  ...
  async generalChatToolCall(input:{agent:Record<string,unknown>;message:string;history:Array<{role:string;content:string}>}){
    ...
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:prompt.model.name,temperature:prompt.model.temperature,max_output_tokens:prompt.model.maxOutputTokens,input:[{role:'system',content:this.generalAgentInstructions(prompt.systemPrompt,input.agent)},{role:'user',content:JSON.stringify({message:input.message,history:input.history.slice(-8)})}],tools:this.generalChatTools(),tool_choice:'auto'})});
    if(!response.ok)throw await this.openAiToolError(response); const data:any=await response.json();
  ...
  async generalChatToolFinal(input:{agent:Record<string,unknown>;message:string;history:Array<{role:string;content:string}>;toolResults:any[];previousResponseId:string|null}){
    ...
    const fallback=input.toolResults.some(x=>x.call_id==='controlled-fallback'),outputs=input.toolResults.map(x=>({type:'function_call_output',call_id:x.call_id,output:JSON.stringify(x.result)}));
    const evidence={role:'user',content:JSON.stringify({message:input.message,history:input.history.slice(-8),controlled_tool_results:input.toolResults.map(x=>({tool_key:x.name,result:x.result})),rule:'Responda somente com os resultados controlados fornecidos.'})};
    const body:any={model:prompt.model.name,temperature:prompt.model.temperature,max_output_tokens:prompt.model.maxOutputTokens,input:fallback?[evidence]:outputs.length?outputs:[evidence],instructions:this.generalAgentInstructions(prompt.systemPrompt,input.agent)}; if(input.previousResponseId&&!fallback)body.previous_response_id=input.previousResponseId;
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify(body)});if(!response.ok)throw await this.openAiToolError(response);const data:any=await response.json(), usage=data.usage||{};return {answer:this.responseText(data)||GENERAL_CHAT_FALLBACK,model_provider:'openai' as const,model_name:prompt.model.name,input_tokens:usage.input_tokens||0,output_tokens:usage.output_tokens||0,total_tokens:usage.total_tokens||0,dry_run:false,llm_final_called:true};
  }
  async generalChatToolContinue(input:{agent:Record<string,unknown>;toolResults:any[];previousResponseId:string}){const prompt=this.prompts.build({agent:input.agent,runType:'general_chat',allowedTools:GENERAL_CHAT_FUNCTION_NAMES,context:'chat'});const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:prompt.model.name,previous_response_id:input.previousResponseId,input:input.toolResults.map(x=>({type:'function_call_output',call_id:x.call_id,output:JSON.stringify(x.result)})),tools:this.generalChatTools(),tool_choice:'auto',instructions:this.generalAgentInstructions(prompt.systemPrompt,input.agent)})});if(!response.ok)throw await this.openAiToolError(response);const data:any=await response.json();return {calls:(data.output||[]).filter((x:any)=>x.type==='function_call').map((x:any)=>({id:x.call_id||x.id,name:x.name,arguments:this.json(x.arguments)})),response_id:data.id||null,answer:this.responseText(data),done:!(data.output||[]).some((x:any)=>x.type==='function_call')};}
  ...
  private async openAiToolError(response:Response){const errorText=await response.text();const error=new Error(`OpenAI respondeu HTTP ${response.status}`);Object.assign(error,{generalChatGatewayFailure:{stage_failed:'openai_tool_selection',error_code:'openai_response_http_error',http_status:response.status,error_message_safe:this.safeProviderError(errorText)}});return error;}
  private safeProviderError(value:string){let message=value;try{const parsed=JSON.parse(value), error=parsed?.error??parsed;if(error&&typeof error==='object'&&typeof error.message==='string')message=error.message;}catch{}const safe=message.replace(/Bearer\s+[A-Za-z0-9._-]+/gi,'Bearer [redacted]').replace(/sk-[A-Za-z0-9_-]+/gi,'[redacted]').replace(/\s+/g,' ').trim();return (safe||'O provedor não informou detalhes.').slice(0,500);}
  private generalAgentInstructions(base='',agent:Record<string,unknown>={}){return `${base}\nVocê é o Analista Logístico do cliente. Responda diretamente quando não precisar de dados. Use apenas as ferramentas oferecidas para dados. Nunca calcule indicadores fora do mapa analítico, invente números, peça SQL/tabelas/campos técnicos ou exponha dados internos. Perguntas amplas de performance, panorama operacional, dashboard ou relatório recebem composição padrão do backend: não pergunte qual métrica quando esse pacote já estiver disponível. Transitime já identifica a métrica. Peça esclarecimento somente quando faltar entidade necessária, origem/destino indispensável ou período que não possa ser inferido do contexto. Para consultas pontuais, preencha exatamente o schema da ferramenta e, diante de INVALID_TOOL_ARGUMENTS, corrija os argumentos ou peça apenas o dado indispensável. Interprete os fatos retornados com linguagem funcional e objetiva. Comporte-se conforme as instruções configuradas do agente: ${String(agent.system_instructions||agent.behavior_profile||'')}`;}
  /** Single functional tool contract shared by typed chat and Realtime voice sessions. */
  generalChatTools(){return generalChatToolDefinitions();}

  /** Narrates a bounded official evidence pack; it never selects tools or accesses data. */
