...
export class AiGatewayService {
  ...
  async generalChat(input: { agent: Record<string, unknown>; message: string; history: Array<{role:string;content:string}>; toolResult?: unknown; plan?:unknown; toolResults?:unknown[]; capabilitySummary?:unknown }): Promise<GeneralChatResult> {
    ...
    try {
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({ model: model_name, temperature: Number(input.agent.temperature ?? .2), max_output_tokens: Number(input.agent.max_output_tokens ?? 1200), input: [{ role: 'system', content: 'Responda em português do Brasil, diretamente à pergunta. O pacote de evidências oficiais é a única fonte para dados do tenant: use-o para narrar, diga a fonte usada, consolide fontes convergentes e aponte divergências. Nunca invente dados, fórmulas ou regras. Só pode somar, contar, calcular média simples, ordenar ou achar maior/menor usando valores explicitamente recebidos no pacote e deve informar que o cálculo usou dados oficiais recebidos. Não calcule R$/ton, SLA, score, margem ou métrica composta sem indicador ou regra homologada no pacote. Se houver ambiguity, faça exatamente a confirmação objetiva indicada. Sem evidência do tenant, responda como assistente geral à pergunta simples. Não mencione detalhes técnicos, banco, SQL, chaves, ferramentas ou ações automáticas. Retorne JSON {"answer":"..."}.' }, { role: 'user', content: JSON.stringify({ message: input.message, history: input.history.map(x => ({ role: x.role, content: x.content.slice(0, 1000) })), plan:input.plan, official_evidence_pack: input.toolResult, controlled_tool_results: input.toolResults ?? [input.toolResult], capability_summary:input.capabilitySummary }) }], text: { format: { type: 'json_object' } },
        }),
      });
  ...
  private simulatedChatAnswer(toolResult: unknown) { const result: any = toolResult; if(result?.ambiguity)return result.ambiguity; if(Array.isArray(result?.evidence)&&result.evidence.length){const best=result.evidence[0];return `Encontrei evidência oficial em ${best.source_name}. A resposta deve considerar os dados oficiais recebidos dessa fonte.`} return result?.results?.length ? result.results.map((x:any) => `${x.title}: ${x.chunk}`).join('\n\n') : GENERAL_CHAT_FALLBACK; }
