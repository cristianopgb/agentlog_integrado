...
async function main(){
  ...
  const restoredOrchestrator:any={execute:async()=>{orchestratedDirectly++;return{content:'direto'}}};
  const restoredChat:any=new ChatService(sendDb,restoredGateway,{} as any,restoredOrchestrator);restoredChat.buildOfficialEvidencePack=async()=>({question:'faça um resumo operacional',normalized_question:'',detected_terms:[],detected_entities:{},detected_metrics:[],detected_dimensions:[],evidence:[{source_type:'treated_data_summary',source_name:'Base operacional tratada',source_id:'current',confidence:.9,reason:'oficial',data:{record_count:60}}],ambiguity:null,allowed_calculations:[],denied_calculations:[]});await restoredChat.send('tenant','user','conversation',{message:'faça um resumo operacional'});assert.equal(narrated,1);assert.equal(orchestratedDirectly,0);
