...
async function main(){
  ...
  const inspected=inspectGeneralChatToolContract();assert.equal(inspected.tools.length,15);for(const tool of inspected.tools){const definition=generalChatToolDefinitions().find(item=>item.name===tool.function_name)!;assert.deepEqual(tool.allowed_args.sort(),Object.keys(definition.parameters.properties).sort());}
  ...
  const voiceTonUpdates:any[]=[],voiceTonDb:any={select:async(table:string)=>table==='ai_tools'?[{id:'tool'}]:[{id:'enabled'}],insert:async()=>[],update:async(_table:string,_query:string,value:any)=>{voiceTonUpdates.push(value);return[value];}},voiceTon=new GeneralChatOrchestratorService(sprintGateway,sprintTools,voiceTonDb);await voiceTon.executeRealtimeTool('tenant','agent','indicators_get_result',{name:'Frete total'},'voice-r-ton',{last_user_message:'Faça uma análise do reais tonelada por cliente, por favor.'});assert.equal(sprintCalls.at(-1).key,'indicators.get_result');assert.equal(sprintCalls.at(-1).input.id,'custom-r-ton');assert.equal(voiceTonUpdates.at(-1).output_json.resolved_indicator_name,'R$/ton por Cliente');assert.equal(voiceTonUpdates.at(-1).output_json.user_intent_for_resolution,'Faça uma análise do reais tonelada por cliente, por favor.');assert.match(voiceTonUpdates.at(-1).output_json.technical_trace_intent,/Frete total/);assert.equal(voiceTonUpdates.at(-1).output_json.rejected_tool_key,'indicators.get_result');assert.equal(voiceTonUpdates.at(-1).output_json.rejected_reason,'approximate_indicator_rejected');assert.equal(voiceTonUpdates.at(-1).output_json.reroute_reason,'specific_indicator_overrides_approximate_indicator');
  await voiceTon.executeRealtimeTool('tenant','agent','indicators_get_result',{id:'custom-weight'},'voice-clientes-peso',{last_user_message:'reais tonelada por cliente'});assert.equal(sprintCalls.at(-1).input.id,'custom-r-ton');assert.equal(voiceTonUpdates.at(-1).output_json.rejected_tool_key,'indicators.get_result');assert.equal(voiceTonUpdates.at(-1).output_json.rejected_reason,'approximate_indicator_rejected');assert.equal(voiceTonUpdates.at(-1).output_json.resolved_indicator_name,'R$/ton por Cliente');
  await voiceTon.executeRealtimeTool('tenant','agent','treated_data_search_records',{filters:{customer_name:'Indústria de Alimentos Serra Azul'},limit:5},'voice-transitime',{last_user_message:'Sim, faço transtime médico por entrega do cliente Indústria de Alimentos Serra Azul.'});assert.equal(sprintCalls.at(-1).key,'indicators.get_result');assert.equal(sprintCalls.at(-1).input.id,'custom-transitime');assert.equal(sprintCalls.at(-1).input.filters.customer_name,'Indústria de Alimentos Serra Azul');assert.equal(voiceTonUpdates.at(-1).output_json.resolved_indicator_name,'transitime médio por entrega');assert.equal(voiceTonUpdates.at(-1).output_json.reroute_reason,'specific_indicator_overrides_operational_record');
  ...
  const noCatalogRealtimeUpdates:any[]=[],noCatalogRealtimeDb:any={select:async(table:string,key:string)=>table==='ai_tools'&&String(key).includes('indicators.list_available')?[]:table==='ai_tools'?[{id:'tool'}]:[{id:'enabled'}],insert:async()=>[],update:async(_table:string,_query:string,value:any)=>{noCatalogRealtimeUpdates.push(value);return[value];}},noCatalogRealtime=new GeneralChatOrchestratorService(sprintGateway,sprintTools,noCatalogRealtimeDb),noCatalogVoice:any=await noCatalogRealtime.executeRealtimeTool('tenant','agent','dashboard_get_snapshot',{},'voice-no-catalog',{last_user_message:'reais tonelada por cliente'});assert.equal(noCatalogVoice.error_code,'INDICATOR_CATALOG_NOT_AVAILABLE');assert.equal(noCatalogRealtimeUpdates.at(-1).output_json.error_code,'indicator_catalog_not_available');assert.equal(noCatalogRealtimeUpdates.at(-1).output_json.resolved_source_type,'unavailable');
  ...
  const daysExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'days-id',name:'transitime med por carga',status:'active',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:7,value_format:'days',records_used:12,data_quality_notes:[]})} as any,{} as any);const daysTool:any=await daysExecutor.execute('tenant','indicators.get_result',{id:'days-id'});assert.equal(daysTool.display_value,'7 dias');assert.equal(daysTool.primary_display_value,'7 dias');
  const singularDayExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'day-id',name:'transitime med por carga',status:'active',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:1,value_format:'days',records_used:1,data_quality_notes:[]})} as any,{} as any);const singularDayTool:any=await singularDayExecutor.execute('tenant','indicators.get_result',{id:'day-id'});assert.equal(singularDayTool.display_value,'1 dia');
  const perTonExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'per-ton-id',name:'R$/ton por Cliente',status:'active',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:0.862922640739539,value_format:'currency',records_used:5,data_quality_notes:[]})} as any,{} as any);const perTonTool:any=await perTonExecutor.execute('tenant','indicators.get_result',{id:'per-ton-id'});assert.equal(perTonTool.display_value,'R$ 0,86/t');assert.equal(perTonTool.primary_display_value,'R$ 0,86/t');
  const singleMatrixExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'single-matrix-id',name:'transitime médio por entrega',status:'active',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:null,value_format:'days',records_used:1,rows:[{label:'Industria Alimenticia Serra Azul Ltda','DOC-2026-000007':null,'DOC-2026-000055':6,'DOC-2026-000055_used_count':1}],data_quality_notes:[]})} as any,{} as any);const singleMatrixTool:any=await singleMatrixExecutor.execute('tenant','indicators.get_result',{id:'single-matrix-id'});assert.equal(singleMatrixTool.value,6);assert.equal(singleMatrixTool.primary_value,6);assert.equal(singleMatrixTool.display_value,'6 dias');assert.equal(singleMatrixTool.primary_display_value,'6 dias');
  const multiMatrixExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'multi-matrix-id',name:'transitime médio por entrega',status:'active',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:null,value_format:'days',records_used:2,rows:[{label:'Cliente A','DOC-1':5,'DOC-2':7}],data_quality_notes:[]})} as any,{} as any);const multiMatrixTool:any=await multiMatrixExecutor.execute('tenant','indicators.get_result',{id:'multi-matrix-id'});assert.equal(multiMatrixTool.value,null);assert.deepEqual(multiMatrixTool.rows,[{label:'Cliente A','DOC-1':5,'DOC-2':7}]);
  const technicalOnlyExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'technical-only-id',name:'transitime médio por entrega',status:'active',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:null,value_format:'days',records_used:5,rows:[{label:'Cliente A',used_count:5,records_used_in_group:5,records:5}],data_quality_notes:[]})} as any,{} as any);const technicalOnlyTool:any=await technicalOnlyExecutor.execute('tenant','indicators.get_result',{id:'technical-only-id'});assert.equal(technicalOnlyTool.value,null);
