...
async function main(){
  ...
  const tabularExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'tabular-id',name:'R$/ton por Cliente',status:'active',value_format:'currency_per_ton',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:null,display_value:null,table:[{label:'Rede Mercantil Boa Compra Ltda',value:123.45,records:5}],records_used:5,data_quality_notes:[]})} as any,{} as any);const tabularResult:any=await tabularExecutor.execute('tenant','indicators.get_result',{id:'tabular-id',filters:{customer_name:'Rede Mercantil Boa Compra Ltda'}});assert.equal(tabularResult.status,'available');assert.equal(tabularResult.value,123.45);assert.equal(tabularResult.primary_value,123.45);assert.equal(tabularResult.display_value,'R$ 123,45/t');assert.deepEqual(tabularResult.rows,[{label:'Rede Mercantil Boa Compra Ltda',value:123.45,records:5}]);assert.deepEqual(tabularResult.result_rows,tabularResult.rows);const compactedTabular=(sprint as any).compact(tabularResult);assert.equal(compactedTabular.table,undefined);assert.deepEqual(compactedTabular.rows,[{label:'Rede Mercantil Boa Compra Ltda',value:123.45,records:5}]);assert.deepEqual(compactedTabular.result_rows,compactedTabular.rows);
  const seriesExecutor=new AgentToolExecutorService({} as any,{list:async()=>({data:[]})} as any,{list:async()=>({data:[{id:'series-id',name:'transitime médio por entrega',status:'active',value_format:'days',available_for_dashboard:true,available_for_reports:true}]}),previewSaved:async()=>({status:'success',value:null,table:[],series:[{label:'Entrega 1',total:2.5,records:1}],records_used:1,records_ignored_missing_data:4,data_quality_notes:[]})} as any,{} as any);const seriesResult:any=await seriesExecutor.execute('tenant','indicators.get_result',{id:'series-id',filters:{customer_name:'Rede Mercantil Boa Compra Ltda'}});assert.equal(seriesResult.status,'available');assert.equal(seriesResult.records_used,1);assert.equal(seriesResult.records_ignored_missing_data,4);assert.equal(seriesResult.value,2.5);assert.equal(seriesResult.display_value,'2,5 dias');assert.deepEqual(seriesResult.rows,[{label:'Entrega 1',total:2.5,records:1}]);assert.deepEqual((sprint as any).compact({rows:seriesResult.rows,result_rows:seriesResult.result_rows}).rows,seriesResult.rows);
  const dashboardToolDb:any={select:async(table:string)=>table==='dashboard_definitions'?[{id:'dash-id',title:'Dashboard Sprint 17',published_version_id:'version-id'}]:[],insert:async()=>[],update:async()=>[]},dashboardPublished:any={load:async()=>({version:{id:'version-id'},widgets:[{id:'widget-table',title:'Tabela de clientes',visual_type:'table',indicator_id:'indicator-table'},{id:'widget-matrix',title:'Matriz de clientes',visual_type:'matrix',indicator_id:'indicator-matrix'}]}),previewWidget:async(_t:string,widget:any)=>widget.visual_type==='matrix'?{status:'success',rows:[{label:'Cliente A',value:20}]}:{status:'success',rows:[{label:'Cliente B',value:10}]}};const dashboardExecutor=new AgentToolExecutorService(dashboardToolDb,{} as any,{} as any,dashboardPublished);const dashboardSnapshot:any=await dashboardExecutor.execute('tenant','dashboard.get_snapshot',{dashboard_id:'latest'});assert.equal(dashboardSnapshot.found,true);assert.equal(dashboardSnapshot.structured_results.length,2);assert.equal(dashboardSnapshot.matrix_results.length,1);const compactedDashboard=(sprint as any).compact(dashboardSnapshot);assert.equal(compactedDashboard.tables,undefined);assert.equal(compactedDashboard.structured_results.length,2);assert.equal(compactedDashboard.matrix_results.length,1);
