...
import { AgentSemanticCatalogService } from './agent-semantic-catalog.service';
import { AgentQueryPlannerService } from './agent-query-planner.service';
import { ControlledAgentQueryExecutorService } from './controlled-agent-query-executor.service';
...
export class ChatService {
  constructor(private db: SupabaseService, private gateway: AiGatewayService, private tools: AgentToolExecutorService, private catalog:AgentSemanticCatalogService, private planner:AgentQueryPlannerService, private executor:ControlledAgentQueryExecutorService) {}
  ...
  async send(t:string,u:string,id:string,b:Record<string,unknown>){
    ...
    try {
      const history=await this.recentHistory(t,id), capabilityCatalog=await this.catalog.build(t), plan=await this.planner.plan({tenantId:t,userId:u,message,recentMessages:history,capabilityCatalog});
      if(plan.intent==='clarification')return this.complete(t,id,c,run,agent,plan.clarification_question||'Pode detalhar a consulta?',undefined,{dry_run:true,model_provider:'system',model_name:'system'},true,undefined,plan,[],{});
      const requiredTools=this.executor.toolKeys(plan); for(const key of requiredTools)if(!(await this.enabled(t,agent.id,key))){await this.recordUnavailableTool(t,run.id,key,{intent:plan.intent});return this.complete(t,id,c,run,agent,`Essa consulta exige a ferramenta ${key}, mas ela não está habilitada para este agente.`,key,{dry_run:true,model_provider:'system',model_name:'system'},false,undefined,plan,[],{has_data:false});}
      const executed=await this.executor.execute(t,plan); if(executed.length>5)return this.complete(t,id,c,run,agent,'Para responder com segurança, informe um escopo menor para a consulta.',undefined,undefined,true,undefined,plan,[],{});
      const toolResults:any[]=[]; for(const action of executed){const output=await this.call(t,run.id,action.tool_key,plan as any,action.result);toolResults.push({tool_key:action.tool_key,result:output});}
      const primaryOrder=['indicators.get_result','treated_data.aggregate_records','treated_data.get_entity_performance','treated_data.get_operational_overview','treated_data.list_records','knowledge_base.search'];
      const primary=primaryOrder.map(key=>toolResults.find(x=>x.tool_key===key)).find(Boolean)||toolResults[0];
      let toolResult:unknown=primary?.result??null, toolKey=toolResults.map(x=>x.tool_key).join(','), knowledgeResultsCount:number|undefined;
      if(toolKey==='knowledge_base.search') knowledgeResultsCount=Array.isArray((toolResult as any).results)?(toolResult as any).results.length:0;
      const result=await this.gateway.generalChat({agent,message,history,toolResult,plan,toolResults,capabilitySummary:{entities:capabilityCatalog.entities,indicators:capabilityCatalog.indicators.length,knowledge_documents:capabilityCatalog.knowledge.document_count}});
      ...
      return this.complete(t,id,c,run,agent,answer,toolKey,result,fallbackUsed,knowledgeResultsCount,plan,toolResults,{has_data:toolResults.length>0,primary_tool_key:primary?.tool_key});
    } catch(e) {
  ...
  private async complete(t:string,id:string,c:any,run:any,agent:any,answer:string,toolKey?:string,result?:any,fallbackUsed=false,knowledgeResultsCount?:number,plan?:unknown,toolResults?:unknown[],resultSummary?:unknown){
    ...
    const output={answer,tool_key:toolKey,plan,tools_called:(toolResults||[]).map((x:any)=>x.tool_key),primary_tool_key:(resultSummary as any)?.primary_tool_key??null,result_summary:resultSummary,...(toolKey==='knowledge_base.search'?{knowledge_results_count:knowledgeResultsCount??0}:{}),...(fallbackUsed?{fallback_used:true,reason:'empty_or_invalid_gateway_answer'}:{})};
  ...
  private async call(t:string,run:string,key:string,input:Record<string,unknown>,precomputed?:unknown){const started=Date.now(),output=precomputed===undefined?await this.tools.execute(t,key,input):precomputed,tools=await this.db.select<any[]>('ai_tools',`tool_key=eq.${key}&limit=1`);await this.db.insert('ai_tool_calls',{tenant_id:t,ai_run_id:run,tool_id:tools[0]?.id??null,tool_key:key,input_json:{intent:input.intent,filters:input.filters},output_json:output,status:'completed',duration_ms:Date.now()-started,finished_at:new Date().toISOString()});return output}
  ...
  async voiceMessage(t:string,u:string,id:string,b:Record<string,unknown>){await this.conversation(t,u,id);const transcript=await this.gateway.transcribe(String(b.audio_base64||''),String(b.audio_mime_type||'audio/webm'),String(b.language||'pt-BR'));const response=await this.send(t,u,id,{message:transcript});const audio=await this.gateway.speech(String(response.message?.content||FALLBACK));return {transcript,message:response.message,audio_base64:audio.audio_base64,audio_mime_type:audio.audio_mime_type,dry_run:audio.dry_run};}
