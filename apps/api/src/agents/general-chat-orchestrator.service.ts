...
export class GeneralChatOrchestratorService {
  ...
  async execute(tenantId:string,agent:any,message:string,history:any[]=[],runId?:string){
    ...
    while(rounds++<3&&state.calls?.length&&results.length<4){const batch=state.calls.slice(0,4-results.length);const outputs=[] as any[];for(const call of batch){const name=this.internalName(call.name),args=this.validateCall(name,call.arguments),result=await this.executeTool(tenantId,agent.id,name,args,runId),entry={call_id:call.id,name,result:this.compact(result)};results.push(entry);outputs.push(entry);}state=await this.gateway.generalChatToolContinue({agent,toolResults:outputs,previousResponseId:state.response_id});if(state.done){final={answer:state.answer,model_provider:'openai',llm_final_called:true};break;}}
  ...
  private validateCall(name:string,args:unknown){
    ...
    const allowed:Record<string,string[]>={
      'analytics.map.get':['scope','search'], 'analytics.result.get':['result_key','period','filters','include'],
      'analytics.context.analyze':['context_type','context_value','period','include'],
      'operational.record.find':['identifier_type','identifier_value','filters','period','limit'],
      'knowledge.guidance.search':['topic','user_question','limit']
    };
    ...
    if(value.filters)this.filters(name,value.filters);
    if(name==='knowledge.guidance.search'&&(typeof value.topic!=='string'||typeof value.user_question!=='string'))throw new BadRequestException('Orientação inválida.');
    if(value.period) this.period(value.period); return value;
  }
  ...
  private async executeTool(t:string,agentId:string,key:string,args:Record<string,any>,runId?:string){
    const enabled=await this.db.select<any[]>('ai_agent_tools',`select=id&tenant_id=eq.${t}&agent_id=eq.${agentId}&is_enabled=eq.true&ai_tools!inner(tool_key)&ai_tools.tool_key=eq.${key}&limit=1`);
    if(!enabled[0])throw new BadRequestException('Ferramenta não habilitada para este agente.');
    const started=Date.now(); try {const result=await this.tools.execute(t,key,args);await this.log(t,key,args,result,'completed',started,runId);return result;}catch(error){await this.log(t,key,args,{message:'Falha controlada ao consultar dados.'},'failed',started,runId);throw error;}
  }
  private async log(t:string,key:string,input:any,output:any,status:'completed'|'failed',started:number,runId?:string){if(!runId)return;const [tool]=await this.db.select<any[]>('ai_tools',`select=id&tool_key=eq.${key}&limit=1`);await this.db.insert('ai_tool_calls',{tenant_id:t,ai_run_id:runId,tool_id:tool?.id??null,tool_key:key,input_json:input,output_json:this.compact(output),status,duration_ms:Date.now()-started,finished_at:new Date().toISOString()});}
