...
export class GeneralChatOrchestratorService {
  ...
  async execute(tenantId:string,agent:any,message:string,history:any[]=[],runId?:string){
    ...
    try {
      ...
      while(rounds++<3&&state.calls?.length&&results.length<4){const batch=state.calls.slice(0,4-results.length);const outputs=[] as any[];for(const call of batch){const name=this.internalName(call.name);if(keys.includes(name as any))toolKeys.push(name);let args:Record<string,any>;try{args=this.validateCall(name,call.arguments);}catch(error){if(keys.includes(name as any)){const result=this.invalidArgumentsResult();await this.log(tenantId,name,call.arguments,result,'blocked',Date.now(),runId);const entry={call_id:call.id,name,result};results.push(entry);outputs.push(entry);continue;}throw error;}const result=await this.executeTool(tenantId,agent.id,name,args,runId),entry={call_id:call.id,name,result:this.compact(result)};results.push(entry);outputs.push(entry);}state=await this.gateway.generalChatToolContinue({agent,toolResults:outputs,previousResponseId:state.response_id});if(state.done){final={answer:state.answer,model_provider:'openai',llm_final_called:true};break;}}
  ...
  private validateCall(name:string,args:unknown){
    ...
    const allowed:Record<string,string[]>={'analytics.map.get':['search'],'analytics.result.get':['result_key','metric_key','breakdown_by','period','filters'],'analytics.context.analyze':['context_type','context_value','period'],'operational.record.find':['identifier_type','identifier_value','filters','period','limit'],'knowledge.guidance.search':['topic','user_question','limit']};
  ...
  private invalidArgumentsResult(){return {tool_error:true,error_code:'INVALID_TOOL_ARGUMENTS',message:'Parâmetros inválidos para esta ferramenta.',allowed_schema_summary:{filters:['customer_name','shipper_name','driver_name','vehicle_plate','status','origin_state','origin_city','destination_state','destination_city'],metric_key:['frete_total','frete_medio','peso_total','volume_total','total_entregas','entregas_atrasadas','entregas_canceladas'],breakdown_by:['status','customer_name','shipper_name','driver_name','vehicle_plate','route','origin','destination']},retry_instruction:'Refaça a chamada usando apenas os nomes permitidos no schema.'};}
