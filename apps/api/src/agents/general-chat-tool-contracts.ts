...
export function generalChatToolDefinitions(){return [
  ...
  fn('indicators_get_result','Consulta o resultado oficial de um indicador disponível, com recorte e comparação controlados.',{id:{type:'string'},name:{type:'string'},filters,compare:{type:'boolean'} }),
  ...
  fn('occurrences_analytics_list','Lista até vinte ocorrências na visão analítica tratada.',{filters:{type:'object',properties:{status_scope:{type:'string',enum:['open','closed','all']},current_status:{type:'string',enum:['open','triage','partially_resolved','resolved','closed']},current_priority:{type:'string'},source_channel:{type:'string'},sla_status:{type:'string'},has_pending_actions:{type:'boolean'},has_operation_link:{type:'boolean'},reason_name:{type:'string'},reason_category:{type:'string'},linked_document_number:{type:'string'},linked_delivery_number:{type:'string'},opened_from:{type:'string'},opened_to:{type:'string'}},additionalProperties:false},limit:{type:'integer',minimum:1,maximum:20}}),
  fn('occurrences_analytics_detail','Detalha uma ocorrência por identificador canônico ou referência externa tratada.',{identifier:{type:'string',pattern:'^[A-Za-z0-9_./-]{1,60}$'},identifier_type:{type:'string',enum:['auto','agentlog_number','external_number','linked_document_number','linked_delivery_number']}},['identifier']),
];}
