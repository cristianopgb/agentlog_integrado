...
export class AttendanceAgentToolsService {
  ...
  async execute(tenantId:string,key:string,args:Args,actorId:string){
    ...
    const handlers:Record<string,()=>Promise<unknown>>={
      ...
      'attendance.occurrence.reasons.list':()=>this.listReasons(tenantId,args),
  ...
  private async findContact(t:string,a:Args){const p=phone(a.phone),rows=await this.db.select<any[]>('contacts',`select=id,name,phone,contact_type,driver_id,customer_id&tenant_id=eq.${t}&phone=eq.${p}&deleted_at=is.null&limit=1`);if(rows[0]&&typeof a.conversation_id==='string'){const id=text(a.conversation_id,'conversation_id',80),conversations=await this.db.select<any[]>('inbox_conversations',`select=id&tenant_id=eq.${t}&id=eq.${id}&channel=eq.public_chat&deleted_at=is.null&limit=1`);if(conversations[0]){await this.db.update('inbox_conversations',`tenant_id=eq.${t}&id=eq.${id}`,{contact_id:rows[0].id,public_phone_normalized:p});await this.db.update('public_chat_sessions',`tenant_id=eq.${t}&conversation_id=eq.${id}`,{contact_id:rows[0].id,contact_phone:p,contact_name:rows[0].name});}}return rows[0]?{found:true,contact_id:rows[0].id,name:rows[0].name,phone:rows[0].phone,contact_type:rows[0].contact_type,driver_id:rows[0].driver_id,customer_id:rows[0].customer_id}:{found:false};}
  ...
  private async listReasons(t:string,a:Args){const search=typeof a.search==='string'?a.search.trim().slice(0,100):'',rows=await this.db.select<any[]>('occurrence_reasons',`select=id,code,name,category_id,is_active&tenant_id=eq.${t}&is_active=eq.true${search?`&or=(code.ilike.*${encodeURIComponent(search)}*,name.ilike.*${encodeURIComponent(search)}*)`:''}&order=name.asc&limit=30`);return{reasons:rows.map(r=>({reason_id:r.id,reason_code:r.code,reason_name:r.name,reason_category:r.category_id,active:true}))};}
  private async resolveReason(t:string,a:Args){const supplied=typeof a.reason_id==='string'?a.reason_id.trim():'';if(supplied&&uuid.test(supplied)){const rows=await this.db.select<any[]>('occurrence_reasons',`select=id&tenant_id=eq.${t}&id=eq.${supplied}&is_active=eq.true&limit=1`);return rows[0]?.id??null;}const code=typeof a.reason_code==='string'?a.reason_code.trim():supplied&&!uuid.test(supplied)?supplied:'';if(code){const rows=await this.db.select<any[]>('occurrence_reasons',`select=id&tenant_id=eq.${t}&code=ilike.${encodeURIComponent(code)}&is_active=eq.true&limit=1`);if(rows[0])return rows[0].id;}const name=typeof a.reason_name==='string'?a.reason_name.trim():supplied;if(!name)return null;const rows=await this.db.select<any[]>('occurrence_reasons',`select=id&tenant_id=eq.${t}&name=ilike.${encodeURIComponent(name)}&is_active=eq.true&limit=2`);if(rows.length>1)return'needs_clarification';return rows[0]?.id??null;}
  private async createOccurrence(t:string,a:Args,u:string){const conversationId=text(a.conversation_id,'conversation_id',80),conversations=await this.db.select<any[]>('inbox_conversations',`select=id,contact_id&tenant_id=eq.${t}&id=eq.${conversationId}&channel=eq.public_chat&deleted_at=is.null&limit=1`);if(!conversations[0])throw new BadRequestException('Conversa pública não encontrada.');if(a.operation_record_id===conversationId)throw new BadRequestException('conversation_id não pode ser operation_record_id.');if(a.verification_result===false||a.verification_result==='denied')return{created:false,blocked:true,reason:'driver_mismatch'};const reason=await this.resolveReason(t,a);if(reason==='needs_clarification')return{created:false,needs_clarification:true};if(!reason)return{created:false,reason_not_found:true,needs_more_data:['reason_code']};const uncertain=a.verification_result==='uncertain'&&a.verification_reason==='driver_phone_missing',audit=uncertain?'Vínculo motorista/operação não confirmado automaticamente porque a operação não possui telefone tratado.':'Ocorrência criada pelo agente de atendimento.';const occurrence:any=await this.occurrences.create(t,u,{title:a.title,description:a.description,current_priority:a.priority??a.severity??'medium',source_channel:'public_chat',source_reference:conversationId,reason_id:reason,operation_record_ids:a.operation_record_id?[a.operation_record_id]:[],primary_operation_record_id:a.operation_record_id,event_description:audit,metadata:{contact_id:a.contact_id??conversations[0].contact_id,requires_human_review:Boolean(a.requires_human_review)||uncertain,evidence_summary:a.evidence_summary,verification_note:uncertain?audit:undefined}});await this.db.insert('conversation_occurrence_links',{tenant_id:t,conversation_id:conversationId,occurrence_id:occurrence.id,relationship_type:'created_from',created_by:u});return{created:true,occurrence_id:occurrence.id,occurrence_number:occurrence.occurrence_number,status:occurrence.current_status,needs_more_data:[]};}
