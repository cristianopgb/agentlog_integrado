...
export class NativeRecordsService {
  ...
  async filters(tenantId: string, includeArchived = false) {
    ...
    const opFilters = [`select=id,source_staging_batch_id,source_data_source_id`, `tenant_id=eq.${tenantId}`, 'deleted_at=is.null', 'is_current=eq.true', 'canonical_validity_status=eq.valid'];
