...
export class CustomIndicatorsController {
  ...
  @Post('custom-indicators/:indicatorId/preview') @RequirePermission('indicators.preview') previewSaved(@Param('tenantId') tenantId:string,@Param('indicatorId') id:string,@Req() req:AuthenticatedRequest,@Body() body:Record<string,unknown> = {}){return this.service.previewSaved(tenantId,id,req.user.id,body);}
