...
export class MappingService {
  ...
  async createMapping(tenantId: string, contractId: string, userId: string, body: Record<string, unknown>) { const p = this.pick(body, ['data_contract_field_id','canonical_entity_id','canonical_field_id','mapping_type','status','notes']); if (!p.data_contract_field_id || !p.canonical_entity_id || !p.canonical_field_id) throw new BadRequestException('data_contract_field_id, canonical_entity_id and canonical_field_id are required.'); await Promise.all([this.ensure('data_contracts', tenantId, contractId, 'Data contract not found.'), this.ensure('data_contract_fields', tenantId, String(p.data_contract_field_id), 'Data contract field not found.'), this.ensure('canonical_entities', tenantId, String(p.canonical_entity_id), 'Canonical entity not found.'), this.ensure('canonical_fields', tenantId, String(p.canonical_field_id), 'Canonical field not found.')]); return this.supabase.insert('field_mappings', { ...p, tenant_id: tenantId, data_contract_id: contractId, created_by: userId, updated_by: userId }); }
