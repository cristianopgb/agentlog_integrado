...
import { dashboardAiAnalysis, dashboardAiQuestion, getPublishedDashboard, listDashboardNativeFilters, previewDashboard, type Dashboard, type DashboardAnalysis, type DashboardNativeFilter, type DashboardRuntimeFilter, type Widget } from '../../../../lib/dashboards-api';
...
export default function PublishedDashboard() {
  ...
  const [agentOpen,setAgentOpen]=useState(false),[analysis,setAnalysis]=useState<DashboardAnalysis|null>(null),[agentMessage,setAgentMessage]=useState(''),[aiLoading,setAiLoading]=useState(false),[dryRun,setDryRun]=useState(false),[question,setQuestion]=useState(''),[history,setHistory]=useState<Array<{q:string;a:string}>>([]);
  ...
  return <div className="page-stack app-page">
    <SectionHeader eyebrow="Dashboards" title={dash?.title ?? 'Dashboard publicado'} description="Visualização publicada com dados atualizados e configuração controlada pelo sistema." actions={dash ? <div className="flex gap-2"><button className="btn-secondary" onClick={openAgent}>Análise IA</button><StatusBadge tone="success">Publicado</StatusBadge></div> : undefined} />
    ...
    {agentOpen?<div className="fixed inset-0 z-50 bg-slate-950/40 p-4"><div role="dialog" aria-modal="true" className="mx-auto max-h-full max-w-3xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl"><div className="flex justify-between gap-4"><div><h2 className="text-xl font-bold">Agente de Dashboard</h2><p className="text-sm text-slate-500">Análise baseada nos widgets e filtros atuais deste dashboard.</p></div><button onClick={()=>setAgentOpen(false)}>×</button></div>{dryRun?<span className="mt-3 inline-block rounded bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">Simulação</span>:null}{aiLoading&&!analysis?<p className="mt-5">Gerando análise do dashboard...</p>:null}{agentMessage?<p className="mt-5 text-slate-600">{agentMessage}</p>:null}{analysis?<div className="mt-5 space-y-4 text-sm"><p>{analysis.opening}</p><section><b>Resumo executivo</b><p>{analysis.executive_summary}</p></section>{[['Destaques',analysis.highlights],['Pontos de atenção',analysis.attention_points],['Riscos',analysis.risks],['Recomendações',analysis.recommendations],['Notas de qualidade dos dados',analysis.data_quality_notes]].map(([title,items])=><section key={title as string}><b>{title as string}</b><ul className="list-disc pl-5">{(items as string[]).map(x=><li key={x}>{x}</li>)}</ul></section>)}<section><b>Insights por widget</b>{analysis.widget_insights.map(x=><p key={x.widget_id}><strong>{x.widget_title}:</strong> {x.analysis}</p>)}</section></div>:null}<div className="mt-6 border-t pt-4"><label className="field-label">Pergunte sobre esta análise ou sobre os dados deste dashboard</label><div className="flex gap-2"><input className="field-control" value={question} onChange={e=>setQuestion(e.target.value)} maxLength={1000}/><button className="btn-primary" disabled={aiLoading} onClick={ask}>Perguntar</button></div>{history.map((x,i)=><div key={i} className="mt-3 rounded bg-slate-50 p-3"><p><b>Você:</b> {x.q}</p><p><b>Agente:</b> {x.a}</p></div>)}</div></div></div>:null}
