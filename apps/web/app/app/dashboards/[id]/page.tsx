...
import { getWidgetGridStyle, getWidgetMinHeight } from '../../../../lib/widget-layout';
...
export default function PublishedDashboard() {
  ...
  return <div className="page-stack app-page">
    ...
    <section className="grid grid-cols-12 items-start gap-5">{widgets.map((w: Widget) => { const grid = getWidgetGridStyle(w.position, w.visual_type, previews[w.id]); return <Card key={w.id} className="col-span-12 min-w-0 overflow-hidden p-5 md:[grid-column:var(--widget-column)]" style={{ '--widget-column': grid.gridColumn, minHeight: `${getWidgetMinHeight(w.visual_type, previews[w.id])}px` } as React.CSSProperties}><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold tracking-tight text-slate-950">{w.title}</h2><p className="mt-1 text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">{visualLabel(w.visual_type)}</p></div><span className="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700">Indicador</span></div><DashboardWidget widget={w} result={previews[w.id]} /></Card>; })}</section>
