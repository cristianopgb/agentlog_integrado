...
import { getWidgetGridStyle } from '../../../../lib/widget-layout';
...
function AgentAvatar() {
  return <span aria-hidden="true" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm text-blue-700">🤖</span>;
}
...
export default function PublishedDashboard() {
  ...
  return <div className="page-stack app-page">
    ...
    <section className="grid grid-cols-12 gap-5">{widgets.map((w: Widget) => { const grid = getWidgetGridStyle(w.position, w.visual_type, previews[w.id]); return <Card key={w.id} className="col-span-12 min-w-0 overflow-hidden p-5 md:[grid-column:var(--widget-column)] md:[grid-row-start:var(--widget-row)]" style={{ '--widget-column': grid.gridColumn, '--widget-row': grid.gridRowStart, minHeight: `${Math.max(1, w.position.h || 2) * 104}px` } as React.CSSProperties}><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold tracking-tight text-slate-950">{w.title}</h2><p className="mt-1 text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">{visualLabel(w.visual_type)}</p></div><span className="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700">Indicador</span></div><DashboardWidget widget={w} result={previews[w.id]} /></Card>; })}</section>
    {agentOpen ? <div className="fixed inset-0 z-50 bg-slate-950/40 p-4 sm:p-6">
      <div role="dialog" aria-modal="true" aria-labelledby="dashboard-agent-title" className="mx-auto flex h-[calc(100vh-2rem)] max-h-[90vh] max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl sm:h-[min(760px,90vh)]">
        ...
        <div className="relative min-h-0 flex-1 overflow-y-auto bg-slate-50/70 px-5 py-5 sm:px-6">
          <div className="space-y-5 text-sm">
            ...
            {analysis ? <section aria-label="Análise inicial" className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="mb-4 flex items-center gap-2"><AgentAvatar /><div><p className="font-bold text-slate-900">Análise inicial</p><p className="text-xs text-slate-500">Visão do agente sobre o dashboard atual</p></div></div><div className="space-y-4"><p>{analysis.opening}</p><section><b>Resumo executivo</b><p>{analysis.executive_summary}</p></section>{[['Destaques',analysis.highlights],['Pontos de atenção',analysis.attention_points],['Riscos',analysis.risks],['Recomendações',analysis.recommendations],['Notas de qualidade dos dados',analysis.data_quality_notes]].map(([title,items])=><section key={title as string}><b>{title as string}</b><ul className="list-disc pl-5">{(items as string[]).map(x=><li key={x}>{x}</li>)}</ul></section>)}<section><b>Insights por widget</b>{analysis.widget_insights.map(x=><p key={x.widget_id}><strong>{x.widget_title}:</strong> {x.analysis}</p>)}</section><div className="flex flex-wrap gap-2"><button className="btn-secondary" disabled={voiceLoading} onClick={()=>listen([analysis.opening,analysis.executive_summary,...analysis.highlights].join(' '))}>{voiceLoading?'Gerando áudio...':'Ouvir resposta'}</button></div>{audio?<audio className="w-full" controls src={audio}/>:null}</div></section> : null}
