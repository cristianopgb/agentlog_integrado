...
export default function IntegrationSetupPage() {
  ...
  async function load(t: string, chosenEntityId?: string) {
    ...
    if (c) {
      ...
      setMappings(ms);
      setPreview(pv);
      const eid = chosenEntityId || es.find((item) => item.entity_key === c.entity_key)?.id || es[0]?.id || '';
      setEntityId(eid);
      ...
      setMappings([]);
      setPreview(null);
      const eid = chosenEntityId || es[0]?.id || '';
      setEntityId(eid);
      setCanonicalFields(eid ? await listCanonicalFields(t, eid) : []);
    }
  }
  ...
  async function saveConnection(e: FormEvent<HTMLFormElement>) { e.preventDefault(); if (!source) return; const f = new FormData(e.currentTarget); const sourceType = source.source_type; const metadata = sourceType === 'api' ? { sprint_7_1: true, connection_declared: true, api_url: String(f.get('api_url') || ''), method: String(f.get('method') || 'GET'), auth_type: String(f.get('auth_type') || 'none'), secret_placeholder: String(f.get('secret') || '') ? 'configured_not_displayed' : 'not_configured', note: 'Configuração declarativa: nenhuma chamada real para API externa nesta sprint.' } : { sprint_7_1: true, connection_declared: true, file_type: String(f.get('file_type') || 'xlsx'), upload_enabled: false, note: 'Upload real será implementado em sprint futura.' }; if (sourceType === 'api' && !metadata.api_url) return setMsg('Informe a URL da API para salvar a configuração declarativa.'); await updateIntegrationConnection(tenantId, source.id, { name: String(f.get('name') || source.name), source_type: source.source_type, module_key: String(f.get('module_key') || source.module_key), metadata }); await load(tenantId, entityId); setMsg('Conexão declarativa salva. Defina a estrutura de dados para continuar na trilha principal.'); setStep(contract ? 'mapping' : 'connection'); }
  async function createStructure() { if (!source) return; await createInitialDeliveryContract(tenantId, source); await load(tenantId, entityId); setMsg('Estrutura inicial de entregas criada na integração.'); setStep('mapping'); }
  async function useExistingContract() { if (!selectedContractId) return setMsg('Selecione um contrato existente para vincular.'); await linkExistingContractToIntegration(tenantId, selectedContractId, params.id); await load(tenantId, entityId); setMsg('Contrato existente vinculado à integração.'); setStep('mapping'); }
  async function saveMapping(cf: CanonicalField, form: HTMLFormElement) { if (!contract) return; const f = new FormData(form); const fieldId = String(f.get('data_contract_field_id') || ''); if (!fieldId) return; await upsertFieldMapping(tenantId, contract.id, { data_contract_field_id: fieldId, canonical_entity_id: entityId, canonical_field_id: cf.id, mapping_type: String(f.get('mapping_type') || 'direct'), status: 'draft' }); await load(tenantId, entityId); }
