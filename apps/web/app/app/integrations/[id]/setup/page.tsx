...
export default function IntegrationSetupPage() {
  ...
  async function load(t: string, chosenEntityId?: string) {
    ...
    setEntityId(eid);
    setCanonicalFields((await Promise.all(es.map((entity) => listCanonicalFields(t, entity.id)))).flat());
    if (c) {
  ...
  async function changeEntity(eid: string) { setEntityId(eid); }
  ...
  async function saveMapping(cf: CanonicalField, form: HTMLFormElement) { if (!contract) return; const f = new FormData(form); const fieldId = String(f.get('data_contract_field_id') || ''); if (!fieldId) return; try { await upsertFieldMapping(tenantId, contract.id, { data_contract_field_id: fieldId, canonical_entity_id: cf.canonical_entity_id, canonical_field_id: cf.id, mapping_type: String(f.get('mapping_type') || 'direct'), status: 'draft' }); await load(tenantId, entityId); setMsg('Mapeamento salvo.'); } catch { setMsg('Não foi possível salvar o mapeamento. Tente novamente ou revise a integração.'); } }

  const groupedCanonicalEntities = useMemo(() => entities.map((entity) => ({ ...entity, fields: canonicalFields.filter((field) => field.canonical_entity_id === entity.id) })).filter((entity) => entity.fields.length > 0), [entities, canonicalFields]);
  const selectedCanonicalFields = useMemo(() => canonicalFields.filter((field) => field.canonical_entity_id === entityId), [canonicalFields, entityId]);
