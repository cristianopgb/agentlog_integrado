...
const options = [
  ...
  { type: 'database', title: 'Banco', description: 'Placeholder declarativo. Conector legado real não faz parte da Sprint 7.1.' },
...
export default function NewIntegrationPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-8"><SectionHeader eyebrow="Nova integração" title="Escolha o tipo de integração" description="API e Planilha podem ser configuradas de forma declarativa. Banco e Outro ficam preparados sem conector real." />{msg ? <EmptyState title="Nova integração" description={msg} /> : null}<div className="grid gap-4 md:grid-cols-2">{options.map((option) => <Card key={option.type}><div className="flex items-center justify-between"><h2 className="text-xl font-bold">{option.title}</h2>{option.type === 'api' || option.type === 'spreadsheet' ? <StatusBadge tone="info">declarativo</StatusBadge> : <StatusBadge tone="warning">em breve</StatusBadge>}</div><p className="mt-3 min-h-16 text-sm leading-6 text-slate-600">{option.description}</p><button disabled={!hasPermission(perms, 'core.data_sources.create')} onClick={() => create(option.type)} className="mt-5 w-full rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white disabled:bg-slate-300" type="button">Criar integração {option.title}</button></Card>)}</div></div>;
}
