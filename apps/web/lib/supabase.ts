...
function buildRestError(
  ...
  const details = [
    ...
    .join(' ');
  return new Error(
    `Supabase REST ${status}: ${message}${details ? ` (${details})` : ''}`,
  );
}
...
export function createBrowserSupabaseClient() {
  ...
  async function request<T>(
    ...
    if (!response.ok)
      return {
        data: null,
        error: buildRestError(response.status, response.statusText, text),
      };
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new Error('Invalid Supabase JSON response.'),
      };
    }
  }
  ...
  return {
    auth: {
      ...
      async getUser() {
        const { data: user, error } = await request<{
          id: string;
          email?: string;
        }>('/auth/v1/user');
    ...
    from(table: string) {
      ...
      async function run() {
        const query = [
          encodeQueryParam('select', selected),
          ...filters,
          ...orders,
          ...(limitValue === null
            ? []
            : [encodeQueryParam('limit', String(limitValue))]),
        ].join('&');
        const init = insertBody
          ? {
              method: 'POST',
              headers: {
                Prefer: `return=representation${upsertConflict ? ',resolution=merge-duplicates' : ''}`,
              },
      ...
      return {
        ...
        upsert(
          ...
          if (upsertConflict)
            filters.push(encodeQueryParam('on_conflict', upsertConflict));
          return this;
        },
        ...
        in(column: string, values: string[]) {
          filters.push(encodeQueryParam(column, `in.(${values.join(',')})`));
          return this;
        },
