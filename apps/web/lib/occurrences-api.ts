...
export type OccurrenceEvent={id:string;reason_id?:string;event_type:string;event_title:string|null;event_description:string|null;event_at:string;old_status:string|null;new_status:string|null};
export type OccurrenceReason={id:string;category_id:string;code:string;name:string;is_active:boolean};
export type ReasonRequirement={id:string;field_key:string;stage:'opening'|'update'|'resolution'|'closing';is_required:boolean};
...
export const listOccurrenceReasons=(tenant:string)=>api<OccurrenceReason[]>(`/tenants/${tenant}/occurrences/reasons`);
export const reasonRequirements=(tenant:string,reason:string,stage:string)=>api<ReasonRequirement[]>(`/tenants/${tenant}/occurrences/reasons/${reason}/requirements?stage=${stage}`);
export const createOccurrence=(tenant:string,payload:Record<string,unknown>)=>api<Occurrence>(`/tenants/${tenant}/occurrences`,{method:'POST',body:JSON.stringify(payload)});
export const addOccurrenceEvent=(tenant:string,id:string,payload:Record<string,unknown>)=>api<OccurrenceEvent[]>(`/tenants/${tenant}/occurrences/${id}/events`,{method:'POST',body:JSON.stringify(payload)});
