...
export type NativeRecord = Record<string, string | number | boolean | null> & { id: string; updated_at: string | null; data_quality_status: string | null; source_data_source_name?: string | null };
export type NativeRecordFilterSource = { id: string; name: string; status: string | null };
export type NativeRecordFilterBatch = { id: string; data_source_id: string | null; source_name: string | null; label: string; batch_code: string | null; source_reference: string | null; records_count: number; created_at: string | null; received_at: string | null };
export type NativeRecordFilters = { sources: NativeRecordFilterSource[]; batches: NativeRecordFilterBatch[] };
export type NativeRecordList = { data: NativeRecord[]; pagination: { limit: number; offset: number; count: number }; summary: { total: number; complete: number; partial: number; lastNormalization: string | null; note?: string } };
export type NativeExtensions = Record<string, Array<Record<string, unknown>>>;
export type NativeEvent = { id: string; event_type: string | null; event_title: string | null; event_description: string | null; occurred_at: string | null; created_at: string | null; source_system: string | null };
...
export function listNativeRecordFilters(tenantId: string, includeArchived = false) { const p = new URLSearchParams(); if (includeArchived) p.set('include_archived', 'true'); return api<NativeRecordFilters>(`/tenants/${tenantId}/native-records/filters?${p.toString()}`); }
